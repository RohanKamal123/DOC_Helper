-- Seed data for service_id = 'export' (Export Registration Certificate / ERC).
-- Researched 2026-08-08. Issued by the Office of the Chief Controller of
-- Imports & Exports (CCI&E) under the Ministry of Commerce. Fee figures are
-- ranges (they vary by paid-up capital/business type) cross-referenced from
-- multiple legal/consulting-firm sources reporting the same structure; not
-- independently confirmed against an official fee schedule document.
...
-- Office (headquarters; CCI&E also has regional offices not itemized here)
...
SELECT 'export', v.fee_type, v.amount_bdt, 'per registration', v.notes_en, v.notes_bn, s.id
FROM (VALUES
  ('registration_minimum', 5000, 'Initial ERC registration starts around this figure and can go up to ~80,000 BDT depending on paid-up capital/business type, plus 15% VAT. Confirm your category''s exact fee with CCI&E.', 'প্রাথমিক ইআরসি নিবন্ধন প্রায় এই অংক থেকে শুরু হয়ে পরিশোধিত মূলধন/ব্যবসার ধরন অনুযায়ী ~৮০,০০০ টাকা পর্যন্ত হতে পারে, সাথে ১৫% ভ্যাট। আপনার শ্রেণির সঠিক ফি সিসিআইএন্ডই-তে নিশ্চিত করুন।'),
...
SELECT 'export', v.document_en, v.document_bn, v.is_mandatory, v.applies_to, v.sort_order, s.id
FROM (VALUES
...
SELECT 'export', v.step_number, v.title_en, v.title_bn, v.is_online, s.id
FROM (VALUES
  ...
  (3, 'Receive the Export Registration Certificate (ERC), typically within 4-5 working days with complete documentation', 'সম্পূর্ণ কাগজপত্র থাকলে সাধারণত ৪-৫ কর্মদিবসের মধ্যে রপ্তানি নিবন্ধন সনদ (ইআরসি) গ্রহণ করুন', false)
...
INSERT INTO online_portals (service_id, portal_name, url, description_en, description_bn) VALUES
  ('export', 'CCI&E Online License Management (OLM)', 'https://olm.ccie.gov.bd/', 'Certificate verification and related online services.', 'সনদ যাচাই ও সংশ্লিষ্ট অনলাইন সেবা।');
